...
# Author: Masato TOYOSHIMA (phoepsilonix at gmail dot com)
# Author of based source: UTUMI Hirosi (utuhiro78 at yahoo dot co dot jp)
...
from hinshi import hinshi_hantei, hinshi_f, hinshi_t
...
for line in lines:
    ...
    for c in range(len(line)):
        ...
        if hinsi.startswith('#T3') or \
                ...
                hinsi.startswith('#CN'):
            hinshi = hinshi_hantei(hinsi)
            entry = [yomi, hyouki, str(cost_mozc), hinshi]
            l2.append(entry)
...
for i in range(len(lines)):
    ...
    # Mozc 辞書の並びに変更
    entry = [lines[i][0], lines[i][1], lines[i][3], ""]
    l2.append('\t'.join(entry) + '\n')

lines = l2
dict_name = 'user_dic-ut-alt-cannadic'
